      ...
      React.createElement(
        ...
        React.createElement(
          ...
          options[option].map((d) =>
            React.createElement("option", { value: d, key: d }, d.toString())
          )
        ),
...
function makeDataFrom(dataType, series, datums, useR) {
  return Array.from(
    { length: series || Math.max(Math.round(Math.random() * 5), 1) },
    (_, i) => makeSeries(i, dataType, datums, useR)
  );
}
...
function makeSeries(i, dataType, datums, useR) {
  ...
  return {
    label: `Series ${i + 1}`,
    data: Array.from({ length }, (_, j) => {
      let x;

      if (dataType === "ordinal") {
        x = `Ordinal Group ${start + j}`;
      } else if (dataType === "time") {
        x = new Date(startDate.getTime() + 60 * 1000 * 60 * 24 * j);
